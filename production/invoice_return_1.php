...
unset($_SESSION["invoice_returns"]);
...
//        $(this).closest('ul').attr('id');
        ...
        var reason = $(tr).find('td:eq(6)').find('select:eq(0)');
        var number = $(tr).find('td:eq(7)').find('input:eq(0)');
    ...
    function getErrors() {
        ...
        $('#tbl tr').each(function (row, tr) {
            var return_reason_id = $(tr).find('td:eq(6)').find('select:eq(0)');
            var qty = $(tr).find('td:eq(7)').find('input:eq(0)');

            if (return_reason_id.val() && qty.val() > 0) {
                val_tblData = true;
            }
        });
        ...
        if (val_tblData) {
            ...
            errors.push("Nothing selected for returning - Not Selected");
            element.css({"border": "1px solid red"});
        }
    ...
    function getTotal() {
        ...
        $('#tbl tr').each(function (row, tr) {
            ...
            var unit_discount = $(tr).find('td:eq(2)').text();
            var return_reason_id = $(tr).find('td:eq(6)').find('select:eq(0)');
            var qty = $(tr).find('td:eq(7)').find('input:eq(0)');

            if (return_reason_id.val() && qty.val() > 0) {
                var sub_total = (price - unit_discount) * qty.val();
                total += sub_total;
            }
        });
        return total;
...
//        return tblData;
    ...
    function submitData() {
        ...
        $.ajax({
            type: 'POST',
            url: "proccess/invoice_return_proccess.php",
            data: {save: true, invoice_id: invoice_id, note: note, tblData: tblData},
            dataType: 'json',
            async: false,
            success: function (data) {
                location.reload();
