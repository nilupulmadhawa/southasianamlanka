...
<?php include 'connection.php'; $result = mysqli_query($conn,"SELECT * FROM invoice WHERE ID = $inv "); while($row = mysqli_fetch_array($result)){GLOBAL $dat,$discount,$credit;$dat = $row[1];$discount = $row[6];$credit = $row[7];} mysqli_close($conn); ?>
	...
	<div style="width:700px;font-size:15px;margin-top:5px;border:1px solid;margin-left:10px;border-color:#c2c2a3;">
		...
		<div style="width:165px;float:left;text-align:left;">
		...
		<?php if($credit == 0){
		echo "<div style='width:700px;font-size:13px;clear:both;height:802px;'>"; }
		else if($credit == 1){
		echo "<div style='width:700px;font-size:13px;clear:both;height:737px;'>"; }
		 ?>
		 <?php if($credit == 0){
			echo "<div style='width:700px;font-size:13px;clear:both;height:auto;height:750px;'>"; }
			if($credit == 1){
			echo "<div style='width:700px;font-size:13px;clear:both;height:auto;height:665px;'>"; }
			?>
			<?php
	  include 'connection.php';
	  $total = 0;
...
echo"<div style='width:170px;float:left;text-align:right;margin-top:3px;padding-right:5px'>".number_format($p,2)."</div>";
	echo '</div>';
	  ...
	  ?>
		...
		<div style="width:525px;float:left;text-align:right;">
			Sub Total Rs.: </br>
			Discount: <br/>
			Total: <br/>
		</div>
		<div style="width:175px;float:left;text-align:right;margin-top:-20px;">
...
<script>
window.location.assign("InvoicePrintA4(duplicate).php");
</script>
</body>
</html>