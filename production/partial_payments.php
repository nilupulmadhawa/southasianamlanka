        ...
        <div class="x_content">
          <table id="tblMain" class="table table-striped table-bordered dt-responsive nowrap " cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              $objects = Invoice::find_all_has_balance();

              foreach ($objects as $invoice) {

                $paid = PaymentInvoice::find_all_by_invoice_id($invoice->id);

                $invoice_status = 0;

                foreach($paid as $paid_data){

                    if( $paid_data->invoice_id == $invoice->id  && $paid_data->payment_id()->payment_status_id != 3 ){
                        $invoice_status = 1;
                    }

                }

                if(!empty($paid) && $invoice_status == 1){
