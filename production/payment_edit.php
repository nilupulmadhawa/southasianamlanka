...
if (!(isset($_GET["id"]) && $payment = Payment::find_by_id($_GET["id"]))) {
    $payment = new Payment();
}
...
if (isset($_GET["id"])) {
    ...
    if ($rowcount > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $invoice_payment["invoice_id"] = $row['id'];
            $invoice_payment["amount"] = $row['amount'];
            $invoice_payment["balance"] = $row['balance'] + $row['amount'];
                        ...
                        <div class="container-fluid divBackTopTable ">
                            ...
                            <div class="row">
                                <div class="form-group col-md-8 col-sm-8 col-xs-12 ">
                                    ...
                                    <input type="hidden" class="form-control" placeholder="Amount" id="txtInvoiceAmountTemp">
                                </div>
                                <div class="form-group col-md-4 col-sm-4 col-xs-12 ">
                                    <label>Action</label>
...
<script>
    ...
    $("#cmbInvoice").change(function() {
        ...
        $("#txtInvoiceAmountTemp").val(getBalance(invoice_id));
    });
    ...
    function addInvoice() {
        ...
        if (errors === undefined || errors.length === 0) {
            ...
            var balance = $("#txtInvoiceAmountTemp").val();

            $.ajax({
                type: "POST",
                url: "proccess/payment_proccess.php",
                data: {
                    add_invoice_payment: true,
                    invoice_id: invoice_id,
                    amount: amount,
                    balance: balance
                },
    ...
    function getErrors() {
        ...
        if (element_value) {
            ...
            if (element_value == 1) {
                var cheque_errors = getCashErrors();
                if (cheque_errors === undefined || cheque_errors.length === 0) {
                    element.css({
                        "border": "1px solid #ccc"
                    });
