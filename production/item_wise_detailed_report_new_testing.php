                  ...
                  <table id="sata_table" class="table table-bordered " cellspacing="0" width="100%">
                    ...
                    <tbody id="table_body">
                      ...
                      if($data = Initial::find_by_product_id($item_name)){
                        ...
                        $oldqty = $oldqty + $data->qty;
                        echo "<td style='text-align: center;' id='initial_balance'> ".$data->qty." </td>";
                        $price_details = Batch::find_by_product_id_first($data->product_id);
                        echo "<td style='text-align: center;'> - </td>";
                        echo "<td style='text-align: right;'> ".number_format($price_details->cost,2)." </td>";
                        echo "<td style='text-align: right;'> ".number_format($price_details->retail_price,2)." </td>";
                        echo "</tr>";
                        $balanceqty = $data->qty;
                      }
                      ...
                      foreach ($period as $key => $value) {
                        ...
                        foreach (StockAdj::find_all_by_date($current, $item_name) as $datass) {
                          ...
                          echo "<td style='text-align: center;'>".$balanceqty."</td>";
                          $batch_details = Batch::find_by_product_id_last($datass->product_id);
                          echo "<td style='text-align: right;'> ".$batch_details->cost." </td>";
                          echo "<td style='text-align: right;'> ".$batch_details->retail_price." </td>";
                          echo "</tr>";
                        }
                        ...
                        foreach (Invoice::find_all_by_date($current) as $data) {
                          foreach (InvoiceInventory::find_all_by_invoice_id($data->id) as $datas) {
                            if( $datas->inventory_id()->product_id == $item_name ){
                              ...
                              echo "<td style='text-align: center;'>".$balanceqty."</td>";
                              $price_range = Batch::find_by_product_id_last($datas->inventory_id()->product_id);
                            //   if($qty_calculation < $oldqty){
                            ...
                            //   }else{
                            //     echo "<td style='text-align: right;'>".$price_range->cost."</td>";
                            //     echo "<td style='text-align: right;'>".$price_range->retail_price."</td>";
                            //   }
                            
                            
                              if($qty_calculation < $oldqty){
                                echo "<td style='text-align: right;'>".$datas->inventory_id()->batch_id()->cost."</td>";
                                echo "<td style='text-align: right;'>".$datas->price."</td>";
                              }else{
                                echo "<td style='text-align: right;'>".$price_range->cost."</td>";
                                echo "<td style='text-align: right;'>".$datas->price."</td>";
                              }
