...
//                                foreach (Payment::find_all_by_payment_type_id(1) as $payment) {
                                ...
                                <tr id="<?php echo $payment->id; ?>">
                                    ...
                                    <td style='text-align:center;'>
                                        <?php echo $cheque_details->cheque_id()->cheque_no; ?> </td>
                                    <td style='text-align:center;'>
                                        <b><?php echo $cheque_details->cheque_id()->bank_id()->name; ?></b>
                                    </td>
                                    <td style='text-align:center;'>
                                        <?php echo $cheque_details->cheque_id()->date; ?>
                                    </td>
                                    <?php
                                        }else{
                                          ?>
                                    ...
                                    <td>
                                        <form action="payment_prev.php" method="post" target="_blank"
                                            style="float: left;">
                                            <input type="hidden" name="payment_id" value="<?php echo $payment->id ?>" />
                                            <button type="submit" name="view" class="btn btn-primary btn-xs"><i
                                                    class="fa fa-external-link-square"></i> View</button>
                                        </form>

                                        <?php
                                            if ($payment->payment_status_id != 3) {
                                                ?>
                                        <form id="form_cancel" action="proccess/payment_proccess.php" method="post"
                                            style="float: left;">
                                            <input type="hidden" name="payment_id" value="<?php echo $payment->id ?>" />
                                            <button id="<?php echo $payment->id ?>" type="submit" name="cancel"
                                                class="btn btn-danger btn-xs btnCancel"><i class="fa fa-close"></i>
                                                Cancel</button>
                                        </form>
                                        <?php
                                            }
            ...
            formSubmit: {
                ...
                action: function() {
                    ...
                    if (authenticate(password)) {
                        ...
                        if (result) {
                            $("#datatable-responsive").find('#' + element.id).find('td:eq(5)').text(
                                "Cancelled");
