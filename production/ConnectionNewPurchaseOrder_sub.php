...
$count = 0;
$result = mysqli_query($conn,"SELECT * FROM po_sub ");
while($row = mysqli_fetch_array($result)){
  GLOBAL $count;
  $count = $row[0];
}
...
if($potype == 1){
  $sql = "INSERT INTO po_sub (ID,PurchaseDate,PoID,SupplierID,DueDate,po_type,CompanyID)
  VALUES ($count,'$podate','$poid',$supplier,$duedate,$potype,$company)";
  ...
  $sql = "INSERT INTO po_sub (ID,PurchaseDate,PoID,SupplierID,DueDate,po_type,RepID,CompanyID)
  VALUES ($count,'$podate','$poid',$supplier,$duedate,$potype,$repID,$company)";
  if (mysqli_query($conn, $sql)) {
    echo "New record created successfully";
  } else {
    echo "Error: " . $sql . "<br>" . mysqli_error($conn);
...
mysqli_close($conn);
header('location:NewPurchaseOrderSecond_sub.php');
?>
