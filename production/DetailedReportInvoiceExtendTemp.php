...
//              GLOBAL $name;
              ...
              echo "<tr><td></td><td></td><td></td><td></td><td><b>TOTAL</b></td><td><b>Rs. ".number_format($total,2)."</b></td></tr>";
      ...
      <div style="margin-top:25px;">
          <p>Invoice Number: <b><?php if(!isset($_SESSION['invoice'])){echo "No Selected Invoice Number";}else{$invoice =  $_SESSION['invoice']; $invoice = sprintf("%04d", $invoice);echo $invoice;} ?></b></p>
          ...
          while($row = mysqli_fetch_array($result)){
            // GLOBAL $cus,$rep,$Allocated,$Deliverd;
            $cus =$row[4];
            $rep = $row[3];
          ...
          <p><?php
            ...
            $result = mysqli_query($conn,"SELECT * FROM user WHERE User_ID = $rep ");
            while($row = mysqli_fetch_array($result)){
              GLOBAL $deliverName;
